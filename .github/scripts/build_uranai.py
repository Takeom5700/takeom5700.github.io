...
# スコア計算エンジン
#
# 2026-09-16 に判明した問題への対処。それまでスコアは外部スクリプトが
# LLM で毎日書いており、実際の運勢と噛み合っていなかった。
# 9/9〜9/16 の実測で、六星の良し悪しとスコア平均の相関は r=+0.16（ほぼ無関係）。
# 最悪の日（減退/乱気）が平均6.2、最良の日（財成/緑生）が6.3 とほぼ同じで、
# 9/16（緑生/財成★＝好日）の外出欄に「破壊的な出来事に注意」と
# 正反対の文言が入っていた。
#
# そこで、点数も文章も六星の日運・月運と四柱の十神から機械的に導く。
# 同じ日なら必ず同じ結果になり、なぜその点数なのかを必ず説明できる。
# ==========================================================================

# 六星12運気の「日の勢い」。プラスほど攻めてよい日。
UNSEI_SCORE = {
    "立花": 2.0, "財成": 1.6, "達成": 1.6, "安定": 1.0, "緑生": 0.8,
    "種子": 0.3, "再会": 0.3,
    "健弱": -1.4, "陰影": -1.2, "停止": -1.4, "減退": -1.6, "乱気": -1.8,
}

# 十神が各項目に与える影響。四柱推命の一般的な意味づけに沿う。
TEN_GOD_EFFECT = {
    "比肩": {"外出": 0.5, "仕事": 0.5, "発言": 0.3, "ガチャ": -0.3},
    "劫財": {"ガチャ": -1.0, "買い物": -0.8, "恋愛": -0.3, "仕事": 0.3},
    "食神": {"作品": 1.2, "恋愛": 0.8, "健康": 0.6, "買い物": 0.4, "外出": 0.5},
    "傷官": {"作品": 1.5, "勉強": 0.5, "発言": -1.2, "仕事": -0.4},
    "偏財": {"ガチャ": 1.0, "買い物": 0.6, "外出": 0.6, "恋愛": 0.5},
    "正財": {"買い物": 0.8, "仕事": 0.5, "ガチャ": 0.3, "勉強": 0.3},
    "偏官": {"仕事": 0.4, "健康": -1.0, "発言": -0.8, "外出": -0.3},
    "正官": {"仕事": 1.2, "メール": 0.8, "発言": 0.5, "作品": 0.3},
    "偏印": {"勉強": 1.2, "作品": 0.4, "発言": -0.4, "恋愛": -0.3},
    "正印": {"勉強": 1.0, "健康": 0.6, "メール": 0.4},
}
...
# 天中殺の日に特に響く項目
TENCHU_PENALTY = {"ガチャ": -1.0, "買い物": -1.0, "仕事": -0.5, "恋愛": -0.4}
TENCHU_DEFAULT = -0.3

SCORE_BASE = 5.5


def compute_scores(info: dict, month_unsei: tuple[str, str]) -> list[dict]:
    """その日の六星・四柱から10項目の点数とコメントを機械的に出す。"""
    mok, kin = info["mokusei"], info["kinsei"]
    day_q = (UNSEI_SCORE[mok] + UNSEI_SCORE[kin]) / 2
    month_q = (UNSEI_SCORE[month_unsei[0]] + UNSEI_SCORE[month_unsei[1]]) / 2

    stem_god = info["stem_god"]
    branch_god = info["branch_gods"][0][1]

    out = []
    for key, label, weight in SCORE_CATEGORIES:
        v = SCORE_BASE + day_q * weight + month_q * 0.3
        eff = TEN_GOD_EFFECT.get(stem_god, {}).get(key, 0.0)
        eff_b = TEN_GOD_EFFECT.get(branch_god, {}).get(key, 0.0) * 0.5
        v += eff + eff_b
        if info["tenchusatsu"]:
            v += TENCHU_PENALTY.get(key, TENCHU_DEFAULT)
        v = max(1.0, min(9.5, v))
        v = round(v * 2) / 2          # 0.5刻み

        band = "low" if v < 4.5 else ("mid" if v < 6.5 else "high")

        # --- なぜその点数なのかを必ず書く ---
        tone = ("追い風" if day_q > 0.5 else
                "逆風" if day_q < -0.5 else "可もなく不可もなく")
        parts = [f"六星は{unsei_label(mok)}/{unsei_label(kin)}＝{tone}"]
        total_eff = eff + eff_b
        if abs(total_eff) >= 0.25:
            direction = "が後押し" if total_eff > 0 else "が足を引っぱる"
            god = stem_god if abs(eff) >= abs(eff_b) else branch_god
            parts.append(f"四柱は{god}（{TEN_GOD_NOTE[god]}）{direction}")
        if info["tenchusatsu"]:
            parts.append("⚠天中殺日なので決め事と大きな出費は先送り")
        if month_q < -0.5:
            parts.append(f"今月の月運が{unsei_label(month_unsei[0])}"
                         f"/{unsei_label(month_unsei[1])}で慎重に")
        out.append({"key": key, "label": label, "value": v, "band": band,
                    "comment": "。".join(parts) + "。"})
    return out
