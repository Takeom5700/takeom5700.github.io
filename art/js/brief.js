...
const WORD_FORMS = [
  ...
  // **物の名前で引かないこと。** 引くのは「何が起きるか」で、
  // その先が物である必要はない（依頼者「必ずしもこの世に実態があるものだけを
  // モチーフにするわけではない」）。
  //
  // 裂ける・破れる・断たれる → 裂（面そのものが裂け、あいだに地が覗く）
  [['裂', '割れ', '罅', '隙', '断た', '断ち', '絶', '切れ', '別れ', '溝', '亀裂', '破れ', '破り'],
    { rift: 3, hollow: 1 }],
  // 空く・欠ける・無くなる → 孔（図と地が入れ替わる。「無いところ」が主役）
  [['欠け', '穴', '虚', '喪', '抜け', '不在', '留守', '空白', '空い', '空っぽ', '失く', '無く'],
    { hollow: 3, rift: 1 }],
  // 繰り返す・響く・重なる → 反（物ではなく「反響」という関係）
  [['繰り返', '響', '重な', '反復', 'また同じ', '再び', '何度', '残響', '余韻', '写し', 'こだま'],
    { echo: 3, thread: 1 }],
  // 見る・覗く・隔てる → 窓
...
// 揺れ（拍を崩す感じ）を作る語
// **境目を曖昧にするかどうかも記事から取る。**
// 依頼者「グラデーション的だったり、何かと何かの境目が曖昧だったりするような
// 表現も必要であれば選択肢に入れていい」。
// にじむ・溶ける・霧のような語がある記事は、縁が溶ける景を多く持つ。
// 語が1つも無い記事は 0 に寄る——**幾何学的に硬いだけの作品もあってよい**
// （依頼者「そういうときももちろんあってもいい」）。
const WORD_BLUR = ['にじ', '滲', '溶け', '融', '霧', '霞', '朧', '曖昧', '混ざ',
  // **「境」だけを入れないこと。**「境が溶ける」にも「境は明快だ」にも当たるので、
  // 硬いだけの記事が 0.35 まで上がった（実測）。曖昧の側の語で取る。
  '境が曖昧', '境目が', '溶け合', '入り混じ',
  'ぼんや', 'かすか', '微か', '薄れ', '移ろ', '陽炎', '湯気', '煙', 'あわい',
  'うつろ', 'まどろ', '夢', '半ば', 'グラデ', '滑らか', '緩やか'];
...
export function briefFromText(article, seed, opts) {
  ...
  // **境目の曖昧さ**（0=硬い面のまま 1=溶ける景を多く混ぜる）。
  // 語の当たりは一文あたりに直す（そのままだと長い記事ほど振り切れる）。
