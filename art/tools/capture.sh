#!/bin/sh
# 静止画の書き出し。作品を見るためと、検証（tools/measure.mjs）のために使う。
#
#   art/tools/capture.sh 出力.png 秒 [幅x高さ] [蓄積枚数] [歩数] [種] [上書き]
#
# 上書きは検証用の口（例: grain:0,ridge:0.5）。作品の一部ではない。
#
# 同じ引数なら必ず同じ絵になる（基軸「五・種」）。
# 窓の大きさに影響されないよう、キャンバスの中身を直接受け取っている。
...
URL="http://127.0.0.1:$PORT/art/index.html?t=$SEC&frames=$FRAMES&steps=$STEPS&seed=$SEED&w=$W&h=$H&p=$OVER"
mkdir -p "$(dirname "$OUT")"
